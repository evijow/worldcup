...
echo "$($PSQL "select distinct name from teams where team_id in (select winner_id from games where round='Eighth-Final' and year=2014 union select opponent_id from games where round='Eighth-Final' and year=2014) order by name")"
